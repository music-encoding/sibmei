 ...
 * @param {string} [docPath]  name for autogenerated API documentation
 * development
 */
async function buildPlg(sourceFiles, target, develop, docPath) {
  fs.writeFileSync(
    target,
    `${BOM}{
    ${compile(sourceFiles, docPath)}
    ${GLOBALS}
 ...
 * @param {string} [apiDocumentationPath]  If present, documentation about
 *   functions with `export` keyword are written to this path.
 */
function compile(sourceFiles, apiDocumentationPath) {
  const compiledCode = [];
  ...
  compiledCode.push(`ExportedFunctions {"${exportedFunctions.map((f) => f.name).join('" "')}"}`);
  autogenerateDocumentation(exportedFunctions, apiDocumentationPath);
  return compiledCode.join("\n\n");
}

/**
 * @param exportedFunctions {{name: string, parameters: string}[]}
 * @param {string} [apiDocumentationPath]
 */
function autogenerateDocumentation(exportedFunctions, apiDocumentationPath) {
  if (!apiDocumentationPath) {
    return;
  }
  fs.writeFileSync(
    ...
    [
      ...
      "A few additional methods and details on some of the listed methods can be found in the [main extension documentation](Extensions.md).",
      "```js",
      `${exportedFunctions.map((f) => `api.${f.name}${f.parameters}`).sort().join("\n")}`,
      "```",
    ].join("\n")
  );
}
