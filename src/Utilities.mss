...
function BeamIsInterlockingWithTuplet(noteRests) {
    // `noteRests` is a SparseArray of all NoteRests belonging to a beam

    tupletAtStart = noteRests[0].ParentTupletIfAny;
    tupletAtEnd = noteRests[-1].ParentTupletIfAny;
    ...
    firstNoteRestPosition = noteRests[0].Position;
    lastNoteRestPosition = noteRests[-1].Position;
    ...
    followingNoteRest = noteRests[-1].NextItem(noteRests[0].VoiceNumber, 'NoteRest');
    if (null = followingNoteRest)
    {
        followingPosition = noteRests[0].ParentBar.Length;
    }
    else
    {
